...
# Ordered list of canonical context files to inject into the system prompt
...
_SYSTEM_PROMPT_HEADER = """You are the TrueSight DAO Governor Assistant.
...
2. For DAO submissions (Phase 2), require at least one https://github.com/TrueSightDAO/ PR URL.
3. For code changes (future), stop at PR creation unless explicitly told to merge.
4. Never expose secrets, .env values, credentials, or private keys in responses.
5. If unsure, ask the governor rather than guess.
6. Be concise but thorough. Prefer bullet points for lists.
7. When discussing the DAO's mission, reference PURPOSE_AND_MISSION.md.
8. When discussing marketing, reference CMO_SETH_GODIN.md principles.
9. When discussing strategy, reference DR_MANHATTAN.md principles.
...
- create_dao_submission(title, body, pr_urls=[], contributors=None) — submit [CONTRIBUTION EVENT] to Edgar
...
```json
{"proposal": {"action": "create_dao_submission", "title": "...", ...}}
```
...
def build_system_prompt() -> str:
    ...
    if not repo_dir.exists():
        # Fallback: try workspace-relative path for local dev
...
# Cached prompt (rebuild on startup or via sync_context.py)
