...
def submit_ai_agent_contribution(
    ...
    """Wraps dao_client/modules/report_ai_agent_contribution.py.

    Returns dict with {status, stdout, stderr, exit_code}.
    """
    # Locate dao_client in the workspace or expect it on PYTHONPATH
    workspace_root = Path(__file__).resolve().parents[3]
    dao_client_dir = workspace_root / "dao_client"

    # Prefer the installed CLI entry point if available
    ...
    if use_module:
        # Run via python -m from dao_client directory
        cmd = [
            sys.executable,
            "-m",
