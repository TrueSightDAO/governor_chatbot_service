...
def read_repo_file(repo: str, path: str, ref: str = "main") -> dict[str, Any]:
    """Read a file from a TrueSightDAO GitHub repo.

    Args:
        repo: Repo name under TrueSightDAO, e.g. 'dapp', 'tokenomics'
        path: File path in the repo, e.g. 'js/checkout.js'
        ref: Branch or commit. Default: 'main'

    Returns:
        dict with {content: str, size: int, url: str, encoding: str}
        If the path is a directory, returns {entries: [...]} instead.
    """
...
def search_codebase(repo: str, query: str) -> dict[str, Any]:
    """Search for code within a TrueSightDAO repo using GitHub code search.

    Args:
        repo: Repo name under TrueSightDAO
        query: Search string (GitHub code search syntax)

    Returns:
        dict with {total_count: int, items: [...]}
    """
